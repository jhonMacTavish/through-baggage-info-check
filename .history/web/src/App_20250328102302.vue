          ...
          tableData.value.forEach(item => {
            checkData.forEach(checkItem => {
              if (item.FLIGHT_NO_FULL == checkItem.inFlightNo && item.TIME_START_PLAN == dayjs(checkItem.timeStartPlan).format('YYYY-MM-DD HH:mm:ss')) {
                ...
                item.PASSENGER_COUNT != checkItem.passengerTotal || item.BAGGAGE_COUNT != checkItem.piece ? item.warningStyle = true : false;
              }
            })
          })
          const currentTime = dayjs().format('YYYY-MM-DD HH:mm:ss');
          tableData.value.forEach(item => {
            item.PASSENGER_COUNT ? null : item.TIME_START_PLAN <= currentTime ? item.PASSENGER_COUNT = '/' : '';
            item.BAGGAGE_COUNT ? null : item.TIME_START_PLAN <= currentTime ? item.BAGGAGE_COUNT = '/' : '';
            item.PASSENGER_COUNT_WEB ? null : item.TIME_START_PLAN <= currentTime ? item.PASSENGER_COUNT_WEB = '/' : '';
            item.BAGGAGE_COUNT_WEB ? null : item.TIME_START_PLAN <= currentTime ? item.BAGGAGE_COUNT_WEB = '/' : '';
          })
        }
      })
    }
  ...
  try {
    ...
    const createCell = (value, isWarning = false) => {
      const style = isWarning 
        ? { ...centerStyle, fill: { color: { rgb: "FFFF0000" } } }
        : centerStyle

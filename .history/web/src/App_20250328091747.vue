 ...
 * @LastEditTime: 2025-03-28 09:17:46
          ...
          tableData.value.forEach(item => {
            checkData.forEach(checkItem => {
              ...
              if (item.FLIGHT_NO_FULL == checkItem.inFlightNo && item.TIME_START_PLAN == dayjs(checkItem.timeStartPlan).format('YYYY-MM-DD HH:mm:ss')) {
                console.log(item.TIME_START_PLAN <= currentTime);
                item.PASSENGER_COUNT_WEB = checkItem.passengerTotal ? checkItem.passengerTotal : item.TIME_START_PLAN <= currentTime ? '/' : null;
                item.BAGGAGE_COUNT_WEB = checkItem.piece ? checkItem.piece :  item.TIME_START_PLAN <= currentTime ? '/' : null;

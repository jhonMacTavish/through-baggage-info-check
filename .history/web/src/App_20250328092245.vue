 ...
 * @LastEditTime: 2025-03-28 09:22:45
    ...
    if (data.data.length == 0) {
      ...
      await axios.get("/api/statistics/flightInfo").then(res => {
        ...
        if (checkData.length != 0) {
          ...
          tableData.value.forEach(item => {
            item.PASSENGER_COUNT && item.TIME_START_PLAN <= currentTime ? item.PASSENGER_COUNT = '/' : null;
            item.BAGGAGE_COUNT && item.TIME_START_PLAN <= currentTime ? item.PASSENGER_COUNT = '/' : null;
          })
        }
      })
    }

 ...
 * @LastEditTime: 2025-03-28 09:15:22
          ...
          tableData.value.forEach(item => {
            checkData.forEach(checkItem => {
              ...
              if (item.FLIGHT_NO_FULL == checkItem.inFlightNo && item.TIME_START_PLAN == dayjs(checkItem.timeStartPlan).format('YYYY-MM-DD HH:mm:ss')) {
                ...
                item.PASSENGER_COUNT_WEB = checkItem.passengerTotal && item.TIME_START_PLAN <= currentTime ? checkItem.passengerTotal : '/';
                item.BAGGAGE_COUNT_WEB = checkItem.piece && item.TIME_START_PLAN <= currentTime ? checkItem.piece : '/';

 ...
 * @LastEditTime: 2025-03-30 16:16:13
    ...
    if (data.data.length == 0) {
      ...
      await axios.get("/api/statistics/flightInfo").then(res => {
        ...
        if (checkData.length != 0) {
          ...
          tableData.value.forEach(item => {
            item.PASSENGER_COUNT ? null : item.TIME_START_PLAN <= currentTime ? console.log(item.TIME_START_PLAN <= currentTime) : console.log(item.TIME_START_PLAN <= currentTime);

 ...
 * @LastEditTime: 2025-03-28 08:15:59
  ...
  try {
    ...
    if (data.data.length == 0) {
      ...
      tableData.value = data.data.map(item => ({
        ...item,
        TIME_START_PLAN: dayjs(item.计划起飞时间).format('YYYY-MM-DD HH:mm:ss'),
        TIME_TERMINAL_PLAN: dayjs(item.计划到港时间).format('YYYY-MM-DD HH:mm:ss')
      }));
        ...
        if (checkData.length != 0) {
          tableData.value.forEach(item => {
            checkData.forEach(checkItem => {
              console.log(item.FLIGHT_NO_FULL == checkItem.inFlightNo , item.TIME_START_PLAN == dayjs(checkItem.timeStartPlan).format('YYYY-MM-DD HH:mm:ss'))

 ...
 * @LastEditTime: 2025-03-12 15:32:52
  ...
  try {
    await axios.get("/api/queryVerify", { params }).then(res => {
      ...
      verifyData.value = data.map(item =>{
        item.VERIFY_TIME = dayjs(item.VERIFY_TIME).add(8, 'hour').format('YYYY-MM-DD HH:mm:ss');
        return item
      });
    });
  } catch (error) {
      ...
      <el-main>
        ...
        <el-table :data="verifyData" style="width: 100%">
          ...
          <el-table-column label="PASS_PHOTO" >
            <template #default="scope">
              <img :src="`http://10.33.113.23:8002/scims${scope.row.PASS_PHOTO}`" alt="">
            </template>
          </el-table-column>
        </el-table>
      </el-main>

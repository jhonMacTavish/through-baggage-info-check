 ...
 * @LastEditTime: 2025-03-15 11:05:19
...
class Connector {
    ...
    async queryVerify(querys) {
        try {
            ...
            // 校验time参数
            const time = querys.time;
            const tableName = [time <= 7 ? 'TFU_PASS_VERIFICATIONS' : 'TFU_PASS_VERIFICATIONS_HISTORY', time <= 7 ? 'TFU_PASS_BOARDS' : 'TFU_PASS_BOARDS_HISTORY'];

            // 构建动态条件
            const conditions = [];
            const binds = {};
            if (querys.PASSENGER_ID) {
                conditions.push(`TFU_SCIMS.${tableName[0]}.PASSENGER_ID = :passenger_id`);
                binds.passenger_id = querys.PASSENGER_ID;
            }

            const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
            const sql = `SELECT * FROM TFU_SCIMS.${tableName[0]} LEFT JOIN TFU_SCIMS.${tableName[1]} ON TFU_SCIMS.${tableName[0]}.PASSENGER_ID = TFU_SCIMS.${tableName[1]}.PASSENGER_ID ${whereClause}`;

...
class Connector {
    ...
    async queryVerify(querys) {
        try {
            ...
            // 校验time参数
            const time = querys.time;
            const tableName = time < 7 ? 'TFU_PASS_VERIFICATIONS' : 'TFU_PASS_VERIFICATIONS_HISTORY';
    ...
    async queryBoard(querys) {
        try {
            ...
            // 校验time参数
            const time = querys.time;
            const tableName = time < 7 ? 'TFU_PASS_BOARDS' : 'TFU_PASS_BOARDS_HISTORY';

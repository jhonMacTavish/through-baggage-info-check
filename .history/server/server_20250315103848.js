 ...
 * @LastEditTime: 2025-03-15 10:38:48
...
app.get('/api/queryPassenger', async (req, res, next) => {
    try {
        ...
        console.log(querys);
        const rows = await connector.queryPassenger(querys);
...
app.get('/api/queryVerify', async (req, res, next) => {
    try {
        const params = req.query;
        let querys = {};
        Object.keys(params).forEach(key => {
        ...
        console.log(querys);
        const rows = await connector.queryVerify(querys);
        // if (!rows || rows.length === 0) {
        //     return res.status(404).json({
        //         message: `未找到的相关记录`
        //     });
...
app.get('/api/queryBoard', async (req, res, next) => {
    try {
        const params = req.query;
        let querys = {};
        Object.keys(params).forEach(key => {
        ...
        console.log(querys);
        const rows = await connector.queryBoard(querys);
        // if (!rows || rows.length === 0) {
        //     return res.status(404).json({
        //         message: `未找到的相关记录`
        //     });
